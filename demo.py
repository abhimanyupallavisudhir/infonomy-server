...
class InfonomyUser:
    def __init__(self, username: str, email: str, password: str, server_url: str = "http://localhost:8000"):
    ...
    def register(self):
        response = requests.post(f"{self.server_url}/auth/register", json={"username": self.username, "email": self.email, "password": self.password})
        print(f"Registration of {self.username}: {response.status_code}")
        print(response)
    
    def login(self):
        response = requests.post(f"{self.server_url}/auth/jwt/login", data={"username": self.email, "password": self.password})
        self.token = response.json()["access_token"]
        self.session = requests.Session()
        self.session.auth = BearerAuth(self.token)
    
    def create_buyer(self):
        response = self.session.post(f"{self.server_url}/buyers", json={})
        print(f"Create buyer profile of {self.username}: {response.status_code}")
        print(response.json())
    
    def create_seller(self):
        response = self.session.post(f"{self.server_url}/sellers", json={})
